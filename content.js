...
function prepareName() {
  ...
  name.parentElement.appendChild(getNewBlock("Sendor-name-suggestions"));
}

function prepareTicker() {
  const ticker = document.getElementById("ticker");
  ticker.parentElement.parentElement.appendChild(
    getNewBlock("Sendor-ticker-suggestions")
  );
}

function prepareLogo() {
  let showMore = findAllByTextAndCssSelector(
    "image or video",
    "[for=image]"
  )[0];
  showMore.parentElement.appendChild(getNewBlock("Sendor-logo-suggestions"));
}

function findAllByTextAndCssSelector(text, cssSelector) {
  return Array.from(document.querySelectorAll(cssSelector)).filter(
...
function simulateFileSelection(imageInput) {
  ...
  if (imageInput.startsWith("data:image")) {
    ...
    console.error(
      "Invalid input. Please provide a base64 string or a valid HTTPS image URL."
    );
    return;
  }
...
async function startLoadingImageSuggestions() {
  const container = document.getElementById("Sendor-logo-suggestions");
  ...
  try {
    let postId = extractSendorId(xParam);
    ...
    if (data && data.httpUrls) {
      ...
      console.log("No logo suggestions found for this Sendor.");
    }
  } catch (error) {
    console.error("Error fetching logo suggestions:", error);
    document.getElementById("error-logo").textContent =
      "Cant get sendor.pro image suggestions, try again later";
  } finally {
...
async function startLoadingTextSuggestions() {
  const nameContainer = document.getElementById("Sendor-name-suggestions");
  ...
  const tickerContainer = document.getElementById("Sendor-ticker-suggestions");
  ...
  try {
    ...
    if (data && data.tickers) {
      ...
      console.log("No suggestions found for this Sendor.");
    }
  } catch (error) {
    document.getElementById("error-name").textContent =
      "Cant get sendor.pro AI name suggestions, try again later";
    document.getElementById("error-ticker").textContent =
      "Cant get sendor.pro AI ticker suggestions, try again later";
    tickerSpinner.remove();
    nameSpinner.remove();
  }
}
...
async function toggleSendorButtons() {
  if (window.location.hostname === "sendor.pro") {
    if (window.location.pathname === "/post") {
      await waitForElement("#download-extension-btn");
      document.querySelector("#download-extension-btn").remove();
      document
        .querySelector("#launch-coin-btn")
...
const observer = new MutationObserver(addSendorButtons);
